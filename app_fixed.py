...
with st.expander("데이터 범위와 해석 유의사항"):
    st.markdown(
        """
        - 이 데이터는 2024년 자료이며 `내외항구분`은 모두 외항입니다.
        - 따라서 전년 대비 분석과 내항·외항 비교는 현재 데이터만으로 수행할 수 없습니다.
        - `전체물동량`의 공식 단위는 원천 데이터 정의서에서 최종 확인하는 것이 좋습니다.
        - 필터 적용 후 전월 대비는 선택 범위에서 가장 최근 두 달을 비교합니다.
        """
    )
